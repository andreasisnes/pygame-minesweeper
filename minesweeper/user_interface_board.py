...
class UserInterfaceBoard:
    ...
    def draw(self, screen):
        ...
        for i_idx, row in enumerate(self._tiles):
            for j_idx, tile in enumerate(row):
                ...
                y = height - ((len(self._tiles) - i_idx) * tile.get_height()) - self._offset
    ...
    def mouse_up_left(self, y, x):
        if self._click[0] != y or self._click[1] != x:
            ...
            if self._core._tiles[y][x] == core.BoardTile.unopened and self._tiles[y][x] == self._sprites.empty:
    ...
    def mouse_tile(self, x, y):
        ...
        y = (y - (height - (len(self._tiles) * self._sprites.eight.get_width()) - self._offset)) // self._sprites.eight.get_width()
        return y, x
