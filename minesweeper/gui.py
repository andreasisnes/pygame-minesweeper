...
class BoardGame:
    def __init__(self, api, x_offset=0, y_offset=0):
        ...
        self.click = None
        self.board = [[self.tiles[self.api.encoder[x]] for x in y] for y in self.api.board]
    ...
    def draw(self, screen):
        for y, row in enumerate(self.board):
            for x, tile in enumerate(row):
                screen.blit(tile, (tile.get_width() * x + self.x_offset, tile.get_height() * y + self.y_offset))
        ...
        if self.api.is_game_over or self.click is None:
            ...
            if self.api.encoder.is_tile(self.api.board[self.click[0]][self.click[1]]):
                if self.board[self.click[0]][self.click[1]] == self.tiles['0']:
                    self.board[self.click[0]][self.click[1]] = self.tiles[self.api.encoder.ascii_tile]
                elif self.board[self.click[0]][self.click[1]] == self.tiles['*?']:
                    self.board[self.click[0]][self.click[1]] = self.tiles['?']
        else:
            if self.api.encoder.is_tile(self.api.board[y][x]) and self.board[y][x] == self.tiles['0']:
